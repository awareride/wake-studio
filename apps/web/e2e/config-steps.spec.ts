 ...
 *
 * The workspace is grouped into a Phase 1 "Configure" flow (Steps A–D, before
 * Start) and a Phase 2 "Preview" area (after Start). The KWS configuration
 * step (Step C) renders only when the KWS component toggle is enabled.
 */

test('config steps render; KWS step gated on the KWS component toggle', async ({ page }) => {
  await page.goto('/#/workspace')

  // Phase 1 configure flow is present up front.
  ...
  await expect(page.getByText('Step C', { exact: true })).toBeHidden()
  ...
  await expect(page.getByText('Step C', { exact: true })).toBeVisible()
