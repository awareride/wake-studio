...
No network / no GPU: edge-tts is monkeypatched (writes a fake label tree), and
the online HTTP engine is injected with a fake ``post_json``. The canonical
``wake-studio-dataset.zip`` output is re-imported with ``wake_train_kit.dataset``
so the round-trip (generate -> pack -> import -> verify contentHash) is covered.
"""
...
def _fake_edge_tts(phrases, languages, out_dir, samples_per_phrase=3,
    ...
    for phrase in phrases:
        label_dir = out_dir / gen._sanitize_label(phrase)
    ...
    for word in unknown_words or []:
        label_dir = out_dir / gen._sanitize_label(word)
        label_dir.mkdir(parents=True, exist_ok=True)
        (label_dir / f"{languages[0]}_0.wav").write_bytes(b"RIFFfake")
    # the real edge-tts build always writes _background_noise_ silence clips
...
def fake_edge_tts(monkeypatch):
    monkeypatch.setattr(gen, "build_edge_tts_kws_dataset", _fake_edge_tts)
...
def _fake_http_post(payload=None, output_format="pcm16"):
    """Returns a post_json that replies with base64 pcm16 in chat-completions shape."""
    def post(url, headers, payload):
        audio = payload.get("audio") or {}
        fmt = audio.get("format", output_format)
        if fmt == "pcm16":
...
def test_edge_tts_generates_canonical_dataset(tmp_path, fake_edge_tts):
    ...
    # Re-import the produced zip: manifest valid, labels indexed, hash matches.
    ...
    # provenance is the license-gate input
...
# --- online HTTP engine (mimo-http / llm-tts) ----------------------------------

def test_online_http_engine_synthesizes(tmp_path):
    engine = gen.MimoEngine(post_json=_fake_http_post())
...
def test_online_http_full_generate_roundtrip(tmp_path):
    engine = gen.MimoEngine(post_json=_fake_http_post())
    gen.ENGINE_ADAPTERS["mimo-http"] = engine
    try:
        zip_path = gen.generate_dataset(
            {"engine": "mimo-http", "phrases": "hey studio", "languages": "en-US",
        ...
        assert manifest["recipe"]["engine"] == "mimo-http"
    finally:
        gen.ENGINE_ADAPTERS["mimo-http"] = gen.MimoEngine()


def test_online_http_missing_audio_is_clear_error(tmp_path):
    def bad_post(url, headers, payload):
        return {"choices": []}
    engine = gen.MimoEngine(post_json=bad_post)
...
def test_runner_read_params_env():
    params = read_params({"GEN_ENGINE": "mimo-http", "GEN_PHRASES": "hey,hi",
                          "GEN_SAMPLES": "5"})
    assert params["engine"] == "mimo-http"
