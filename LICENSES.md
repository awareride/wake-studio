...
| `kws_streaming` **code** (Traditional streaming-aware models + training, ADR-024 §2.1) | `google-research/google-research` → `kws_streaming` | Apache-2.0 | Yes (code) | ✅ | Repo `LICENSE` (Apache-2.0, Google LLC). Model definitions (dnn…bc_resnet), streaming conversion, and `train/model_train_eval.py`. Used as a **pinned upstream script** (ADR-031), never vendored. |
| `kws_streaming` **pre-trained models** | — | — | — | ✅ n/a | **Upstream ships no weights**: only code + experiment recipes. Anything we train from it is ours and commercially clean, which is why this is the preferred Traditional baseline over the openWakeWord demo classifiers. |
| Speech Commands V1/V2 (training corpus for `kws_streaming`) | `arXiv:1804.03209`, `download.tensorflow.org/data/speech_commands_*` | CC-BY-4.0 | Yes (with attribution) | ✅ | Dataset paper + the dataset's `LICENSE` (CC-BY-4.0, Google). Downloaded by the training job at train time; **never bundled** into the repo or an export. Attribution required in any release trained on it. |
