...
| `kws_streaming` **code** (Traditional streaming-aware models + training, ADR-024 §2.1) | `google-research/google-research` → `kws_streaming` | Apache-2.0 | Yes (code) | ✅ | Repo `LICENSE` (Apache-2.0, Google LLC). Model definitions (dnn…bc_resnet), streaming conversion, and `train/model_train_eval.py`. **Pristine-vendored** at `third_party/kws_streaming` (ADR-037 Tier 3, import at `cf61877d`, #156); the adapter still runs it unmodified (ADR-031), hardening (dep/compat pins) lands as separate commits on the pristine import. |
