...
| Date | Change | Author |
|---|---|---|
| 2026-08-12 | Deliverable 4 notebook rewritten to the **upstream `automatic_model_training_simple` stack** (runs on current Colab's Python 3.11/3.12): `piper-phonemize-cross` + `onnx2tf` instead of the legacy `piper-phonemize` / `tensorflow-cpu==2.8.1` (wheels only to 3.10). openwakeword tracked at `main`, piper-sample-generator pinned to `213d4d5`. Issue #97. | agent |
