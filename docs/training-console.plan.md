...
| 2026-08-14 | §2 implemented (issue #123): the Colab launcher — `wake_training_service.colab_launcher` starts the studio-backend service in the Colab runtime + `cloudflared` (trycloudflare), prints the tunnel URL, and re-prints a fresh URL on reconnect/restart; token printed for Settings → Security; openwakeword notebook gains Step 1.5 (optional tunnel cell, `ENABLE_TUNNEL`/`WAKE_SERVICE_TOKEN` params) while the train adapter lands with Phase 5. | agent |
