...
| 2026-08-11 | ADR-034 composition roots: driver (impl) modules are imported only by wires — host wire `apps/web/src/module-wire.ts` + worker wire `engine/web/worker-wire.ts`, both generated from specs (`scripts/gen-module-wires.mjs`); `BackendModelUrls` shrinks to a driver-opaque bag (embed boot reads the driver-declared urlKey). | agent |
