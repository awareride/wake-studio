...
| 2026-08-25 | **§4.7 provenance inheritance (#210):** dataset `commercialUse` flags inherit into trained bundles — bundle `provenance.json` flips to `research-only` + `commercialUse=false` + `restrictedBy` when any consumed dataset is research-only; `metadata.datasetRefs` records per-dataset id/name/version + license flags (both kws-streaming + openwakeword adapters). Web: bundle importer parses the inherited restriction, `isCommerciallyExportable` is the Phase 4 export-gate input, and colab-import surfaces restricted bundles in-app. | agent |
